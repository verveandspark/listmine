  ...
  const restoreListItem = async (listId: string, itemData: any) => {
    try {
      const { error } = await supabase.from("list_items").insert({
        ...
        due_date: itemData.dueDate ?? itemData.due_date ?? null,
        notes: itemData.notes ?? null,
        assigned_to: itemData.assignedTo ?? itemData.assigned_to ?? null,
        completed: itemData.completed ?? false,
        links: itemData.links ?? null,
        attributes: itemData.attributes ?? null,
        item_order: itemData.order ?? itemData.item_order ?? 0,
        updated_at: new Date().toISOString(),
        last_edited_by_user_id: user?.id || null,
        last_edited_by_email: user?.email || null,
        last_edited_at: new Date().toISOString(),

...
DROP FUNCTION IF EXISTS public.create_list_from_template(UUID, TEXT, UUID);
...
BEGIN
  ...
  v_list_type := CASE p_template_id
    WHEN '17befa2f-0848-4914-8a8a-c0b135402b7d'::uuid THEN 'custom'
    WHEN 'f6f942d1-7467-4d98-b2bf-dcbce6b597d6'::uuid THEN 'custom'
    ELSE v_template.list_type
  END;

  INSERT INTO public.lists (user_id, title, category, list_type, source, template_id, account_id)
  VALUES (v_user_id, p_list_name, v_category, v_list_type, 'template', p_template_id, p_account_id)
  ...
  SELECT 
    ...
    ti.sort_order,
    CASE 
      WHEN v_list_type IN ('grocery-list', 'grocery') AND ti.section IS NOT NULL THEN 
